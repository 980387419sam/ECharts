...
export default React.createClass( {
    ...
    componentDidMount(){
        demo1('eChartsDemo1',this)
    },
    ECharts(){
        return (
            <div id="eChartsDemo1" style={{width:document.body.scrollWidth,height:document.body.scrollHeight}}></div>
        )
    },
    TextA(){
        return (
    ...
    TextC(){
        return (
            <div>
                <h6>多个坐标系的支持</h6>
                <p>ECharts 3 开始独立出了“坐标系”的概念，支持了直角坐标系（catesian，同 grid）、极坐标系（polar）、地理坐标系（geo）。图表可以跨坐标系存在，例如折、柱、散点等图可以放在直角坐标系上，也可以放在极坐标系上，甚至可以放在地理坐标系中。

</p>
                <h6>移动端的优化</h6>
                <p>流量珍贵的移动端需要图表库的体积尽量小。ECharts 和 ZRender 代码的重构，带来了核心部分体积的减小。ECharts 组件众多，并且后面会持续增加，我们提供了更细粒度的按需打包能力。最小体积缩小为 ECharts 2 的 40%。
</p>
            </div>
        )
    },
    ...
    render(){
        return (
            <div>
                <this.ECharts/>
                <div className='eChartsDemo1' style={{width:document.body.scrollWidth*0.7,height:document.body.scrollHeight*0.9}}>
                    <h5>ECharts 特性</h5>
